...
public class ActsSalesMACDFlow {
	...
	private static String getUsernameSalesforce(String testName) throws Exception {
		...
		{
			usernameSalesforce=TestDataManagement.searchDT(1, "envUsernameNameITTQA");
		}
		else
		{
			usernameSalesforce=TestDataManagement.searchDT(0, "envUsernameNameITTQA");
		}
	...
	private static void uploadFile(WebDriver driver, WebDriverWait wait, int stepID) throws Exception 
	{
		String actionName="New MACD Order - 2ndScreen - Update File";
		
		String filePath=TestDataManagement.fileNameD01;
		
		
		Screen macdUploadFiles = new Screen();
		...
		try
		{
			...
			macdUploadFiles.wait(SalesForceSikuli.doneButton);
			
			macdUploadFiles.click(SalesForceSikuli.doneButton);
