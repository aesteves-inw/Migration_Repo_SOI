...
//			Thread.sleep(5000);
	...
	public static boolean checkRecurringPricesVSProductConfiguration(List<TestLog> logStream, WebDriver driver, int stepID,
...
//				System.out.println(productName);
				...
				case "Enterprise Voice":
					...
					List<String> rowsByDescriptions = Arrays.asList(
							"Bizz IP Box - 2B/2V/S4E - One-Time Charge","Bizz IP Box - 2B/2V/S4E - Recurring Charge",
							"Bizz IP Box Other IPBX - One-Time Charge","Bizz IP Box Other IPBX - Recurring Charge","Enterprise Voice - 10 Channels - Recurring Charge",
							"VoIP Individual number - Recurring Charge");

					List<WebElement> rowsLineItems = driver.findElements(By.xpath("//*[@aria-label='Line items and charges']/div[@role='row'][@row='lineItems']"));

					int size = rowsLineItems.size();
					...
					for(int i=1,j=0; (i<=size & j<size); i++,j++) {
						...
						WebElement row = driver.findElements(By.xpath("//*[@aria-label='Line items and charges']/div[@role='row'][@row='lineItems']")).get(i);
						
						System.out.println(row.getText());

						System.out.println(row.getText().contains(rowsByDescriptions.get(j)));
						break;
					}
					break;

				default:
					TestLogger.logDebug(logStream, "checkRecurringPricesForContractTypeSelected", "productName: '" + productName+"' is not valid.");
					return false;
				}
				break;
			...
			default:
				...
				TestLogger.logDebug(logStream, "checkRecurringPricesForContractTypeSelected", "configurationIndex: '" + configurationIndex+"' is not valid.");
				return false;
			}
